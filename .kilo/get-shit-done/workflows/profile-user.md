...
Key references:
- @.kilo/get-shit-done/references/ui-brand.md (display patterns)
- @.kilo/get-shit-done/agents/gsd-user-profiler.md (profiler agent definition)
- @.kilo/get-shit-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH=".kilo/get-shit-done/USER-PROFILE.md"
...
```bash
cp ".kilo/get-shit-done/USER-PROFILE.md" ".kilo/get-shit-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at .kilo/get-shit-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node .kilo/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node .kilo/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `.kilo/get-shit-done/references/user-profiling.md`
...
Reference: @.kilo/get-shit-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node .kilo/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node .kilo/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node .kilo/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to .kilo/get-shit-done/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  ...
  - "Global AGENTS.md" -- "Add profile to .kilo/AGENTS.md for all projects"

**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at .kilo/get-shit-done/USER-PROFILE.md" and jump to step 10.
...
```bash
node .kilo/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd-dev-preferences at .kilo/commands/gsd/dev-preferences.md"

**For AGENTS.md profile section (if selected):**

```bash
node .kilo/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node .kilo/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to .kilo/AGENTS.md"
...
```bash
BACKUP_PATH=".kilo/get-shit-done/USER-PROFILE.backup.md"
```
...
Your profile:    .kilo/get-shit-done/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /gsd-dev-preferences   .kilo/commands/gsd/dev-preferences.md
  ✓ AGENTS.md section       ./AGENTS.md
  ✓ Global AGENTS.md        .kilo/AGENTS.md
```
