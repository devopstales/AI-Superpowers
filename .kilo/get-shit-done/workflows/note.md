...
- **Global scope**: `.kilo/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `.kilo/notes/`)
...
2. Glob `.kilo/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `.kilo/notes/` — works in any directory
