...
<required_reading>
@.kilo/get-shit-done/references/verification-patterns.md
@.kilo/get-shit-done/templates/verification-report.md
</required_reading>
...
```bash
node ".kilo/get-shit-done/bin/gsd-tools.cjs" roadmap analyze
```
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node ".kilo/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node ".kilo/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node ".kilo/get-shit-done/bin/gsd-tools.cjs" verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node ".kilo/get-shit-done/bin/gsd-tools.cjs" verify key-links "$plan")
...
See .kilo/get-shit-done/templates/verification-report.md for complete template.
</step>
