...
```bash
PHASE_INFO=$(node ".kilo/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ".kilo/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node ".kilo/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
